...
```{r fig-sperlingsti, echo=FALSE, fig.cap="喬治．史頗靈使用的字母矩陣。左：3乘3；右：3乘4", fig.show='hold', message=FALSE, warning=FALSE, out.width='30%', paged.print=TRUE}
knitr::include_graphics("figs/iconic_sti.png")
```
...
不同於艾賓浩斯， @Murdock-1962 使用有意義的單字做學習材料，並且徵求多位參與者同時參與一次實驗。運用1960年代已經成熟的幻燈片投影技術，讓參與者以間隔一秒或兩秒的速率，學習一份列表裡的單詞。一次實驗有數量相同的列表十份，每學完一份列表，參與者在有限時間裡自由報告學過的單詞。列表的單詞數量有10個到40個不等，不論是那種長度的列表，總計每個出現次序的平均回答正確率，都會發現如 @fig-serial 的結果，現在多數文獻稱此為序列位置曲線(Serial Position Curve)。

```{r fig-serial, echo=FALSE, fig.show='hold', fig.cap="典型列表學習實驗的序列位置曲線。取自Zwaan et al.(2017)附錄。", out.width='70%', message=FALSE, warning=FALSE, paged.print=TRUE}
...
> [假記憶線上實驗範例](https://www.millisecond.com/download/library/falsememories) <br/>
> **註**：範例網頁由實驗軟體開發公司[Millisecond](https://www.millisecond.com/)提供，選擇視覺版或聽覺版之後，依照網頁指示安裝套件，才能在網頁執行實驗。
...
@Murdock-1962 的序列效應實驗只有進行20項目的情況，雖然改在網頁進行，學習列表項目與測試回憶項目的時間控制， @Zwaan-2018 的設計都與原始實驗一致。初始效應是序列位置1到4與9到12平均正確率差異；新近效應是序列位置12到17與14到20的平均正確率差異。從 @fig-serial-power 來看，再現兩種效應至少需要17人及10人，就能保證90%的考驗力。

```{r fig-serial-power, echo=FALSE, fig.show='asis', message=FALSE, warning=FALSE, paged.print=TRUE, out.width='30%', fig.cap="估計序列效應之考驗力與參與者人數。左圖:初始效應，右圖:前後參與不同刺激"}
knitr::include_graphics("figs/Serial_Position_N.png")
```
