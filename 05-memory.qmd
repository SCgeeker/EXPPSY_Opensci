...
@Zwaan-2018 的假記憶實驗是 @Roediger-1995 的簡化版本：改用視覺呈現字詞項目列表，還有參與者不必做每個列表之後的自由回憶或其他作業，再認作業只需回答有或無。分析重點是未曾學習的列表關鍵詞之再認正確率，預計需要至少10人，才能有90%的考驗力，再現同樣的結果。請參考 @fig鄉fmemory-power。

```{r fig-fmemory-power, echo=FALSE, fig.show='hold', message=FALSE, warning=FALSE, paged.print=TRUE, out.width='50%', fig.cap="估計錯誤記憶誤答率之考驗力與參與者人數。"}
